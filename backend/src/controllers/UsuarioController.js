...
module.exports = {
  ...
  async update(req, res, next) {
    ...
    const token = jwt.sign({documento} , secret, {expiresIn: timer});
    try{     
        var results = await knex('Usuario').withSchema(schemaName).where({ documento : req.body.documento})
        /*.update({
          nome: nome,
          sobrenome: sobrenome,
          email: email,
  ...
  async update_senha(req, res) {
    ...
    try{
    await knex('Usuario').withSchema(schemaName).where({ documento : req.body.documento})
    .update({senha: hash})
